...
parser = argparse.ArgumentParser(description='Download and merge M3U8 segments.')
...
if args.output_name:
    if not args.output_name.endswith('.mp4'):
        args.output_name += '.mp4'
    output_file = os.path.join(args.output_path, args.output_name)
else:
    output_file = os.path.join(args.output_path, f"m3Loader_{int(time.time())}.mp4")

m3_name = output_file.replace('.mp4', '.m3u8')

# Create a unique segments folder
segments_path = os.path.join(args.output_path, f'segments_{int(time.time())}')
...
async def download_m3u8():
    async with aiohttp.ClientSession() as session:
        async with session.get(args.m3u8_url) as response:
...
segment_urls = [urljoin(args.m3u8_url, line.strip()) for line in lines if line and not line.startswith("#")]
...
async def main():
    ...
    async def download_all_segments():
        async with aiohttp.ClientSession() as session:
            tasks = []
            retries = args.retries if args.retries else 0
            if args.progress:
                with tqdm(total=len(segment_urls), desc="Downloading segments") as pbar:
                    for url, name in zip(segment_urls, segment_names):
                        task = download_segment(session, url, name, retries, pbar)
                        tasks.append(task)
                    await asyncio.gather(*tasks)
            else:
                for url, name in zip(segment_urls, segment_names):
                    task = download_segment(session, url, name, retries)
                    tasks.append(task)
                await asyncio.gather(*tasks)
    ...
    try:
        if args.timeout:
            await asyncio.wait_for(download_all_segments(), timeout=args.timeout)
        else:
    ...
    filelist_path = os.path.join(args.output_path, f"filelist_{int(time.time())}.txt")
